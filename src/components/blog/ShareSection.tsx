...
export function ShareSection({ title, url }: ShareSectionProps) {
  ...
  // Customize the share message to write about the author
  const shareMessage = `Check out this insightful post by Deepak Keer: "${title}"`;
      ...
      <Row data-border="rounded" gap="16" horizontal="center" wrap>
        {enabledPlatforms.map((platform, index) => (
          <Button
            ...
            variant="secondary"
            size="s"
            href={platform.generateUrl(shareMessage, url)}
